...
public class advancedOptionsButton : MonoBehaviour {
	...
	void Update()
	{
		...
		if (Input.touchCount == 1 && Input.GetTouch(0).phase == TouchPhase.Ended && canColide == true)
		{
			...
			if (collider2D == Physics2D.OverlapPoint(touchPos))
			{
				...
				isPanning = true;
				googlePlayLogin.googlePlay.leaderBoard = leaderBoard;
				highScoreHandler.renderScore.updateScores(type);
			}
		}else {sprRenderer.sprite = spr2;}

		if (isPanning == true && GameObject.Find ("Student").transform.position.x > -24) {
